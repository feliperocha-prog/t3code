...
const SORT_OPTIONS: ReadonlyArray<{ value: OpenVsxThemeSort; label: string }> = [
  { value: "downloadCount", label: t("Most downloaded") },
  { value: "rating", label: t("Best rated") },
  { value: "timestamp", label: t("Newest") },
    ...
    <section className="space-y-3" aria-labelledby="theme-search-heading">
      ...
      <div className="sr-only" role="status">
        {isSearching
          ...
          : results
            ? results.length === 1
              ? t("{count} supported theme found.", { count: results.length })
              : t("{count} supported themes found.", { count: results.length })
            : ""}
      </div>
        ...
        results.length === 0 ? (
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {results.map((extension) => {
              ...
              const action = isInstalled ? t("Update") : t("Install");
              const progressAction = isInstalled ? t("Updating") : t("Installing");
              return (
                <article
                ...
                >
                  ...
                  <div className="mt-auto flex items-center justify-between gap-2">
                    <div className="flex min-w-0 items-center gap-2">
                      {extension.sourceUrl ? (
                        <Button
                          aria-label={t("View source for {name}", { name: extension.name })}
      ...
      >
        <AlertDialogPopup>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("Update “{name}”?", { name: pendingUpdate?.name ?? "" })}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t(
